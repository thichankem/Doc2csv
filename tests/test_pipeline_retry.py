...
def test_retry_recovers_after_bad_output(tmp_path):
    ...
    p._call_llm = lambda prompt, idx, total, temperature=None, num_predict=None: next(answers)
...
def test_retry_exhausted_returns_raw_fallback(tmp_path):
    ...
    p._call_llm = lambda prompt, idx, total, temperature=None, num_predict=None: "still not json"
    out, ok = p._generate_chunk_json("prompt", 1, 1)
    assert ok is False
...
def test_no_schema_accepts_any_object(tmp_path):
    ...
    p._call_llm = lambda prompt, idx, total, temperature=None, num_predict=None: '{"anything": 1}'
    out, ok = p._generate_chunk_json("prompt", 1, 1)
    assert ok is True
    assert json.loads(out) == {"anything": 1}


def test_retry_increases_num_predict(tmp_path):
    # Truncation is the top cause of bad JSON; retries must raise the token cap.
    p = _mk(tmp_path, num_predict=300, max_json_retries=2)
    seen = []

    def fake(prompt, idx, total, temperature=None, num_predict=None):
        seen.append(num_predict)
        return "still truncated, not json"

    p._call_llm = fake
    p._generate_chunk_json("prompt", 1, 1)
    # attempt 0 uses base; each retry must request strictly more tokens.
    assert seen[0] == 300
